...
# =====================================================
# SECCIÓN 4 — HEATMAP MENSUAL POR ADUANA
# =====================================================
st.subheader("🌡️ Heatmap: Valor por Aduana y Mes")

pivot = evol.pivot_table(
    index="Aduana",
    columns="Mes",
    values="Valor (USD M)",
    aggfunc="sum",
...
fig_heat = go.Figure(go.Heatmap(
    ...
    colorscale="YlOrRd",
    hoverongaps=False,
    hovertemplate="<b>%{y}</b><br>Mes: %{x}<br>Valor: $%{z:,.2f}M USD<extra></extra>",
    colorbar=dict(title="USD M"),
))
fig_heat.update_layout(
    height=max(350, len(pivot) * 35),
    margin={"r": 20, "t": 20, "l": 10, "b": 80},
...
tabla = agg_total.copy().reset_index(drop=True)
tabla.index = tabla.index + 1
tabla["Valor (USD M)"]  = tabla["Valor (USD M)"].map("${:,.2f}M".format)
tabla["Volumen (t)"]    = tabla["Volumen (t)"].map("{:,.1f} t".format)
